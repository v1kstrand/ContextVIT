    ...
    train_transforms = transforms.Compose(
        [
            transforms.RandomResizedCrop(
                size=args.kw["img_size"],
                scale=(0.08, 1.0),
                ratio=(3 / 4, 4 / 3),
            ),
            transforms.RandomHorizontalFlip(p=0.5),
            transforms.RandomApply(
                [transforms.ColorJitter(0.8, 0.8, 0.8, 0.2)], p=0.66
            ),
            ...
            # kernel_size=args.kw["img_size"] * 0.1, sigma=(0.1, 2.0)
            # )
            # ],
            # p=0.1,
            # ),
